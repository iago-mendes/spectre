...
/* SPECTRE-MOD: _Thread_local. SpECTRE calls this from Charm++ SMP worker
   threads; Sc and Bx are written per-interface by HLLD_Fstar /
   HLLD_GetRiemannState and read by the flux assembly. */
static _Thread_local double Sc, Bx;
 ...
 *********************************************************************** */
{
  ...
  int    switch_to_hll;
  double   scrh;
  /* SPECTRE-MOD: _Thread_local scratch (one lazy allocation per thread). */
  static _Thread_local double **Uhll, **Fhll, **Vhll;
