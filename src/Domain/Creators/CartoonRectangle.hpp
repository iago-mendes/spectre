...
/// Create a 3D Domain with its computational domain being the Cartesian
/// \f$x-y\f$ plane. The third (\f$z\f$) dimension uses a Cartoon basis with a
/// TRANSLATIONAL Killing vector (the solution is uniform in \f$z\f$), so it is
/// collapsed to a single grid point. `x` and `y` may be periodic. Intended for
/// effectively-2D Cartesian problems (e.g. Kelvin-Helmholtz).
class CartoonRectangle : public DomainCreator<3> {
 public:
  ...
  static std::string name() { return "CartoonRectangle"; }
  ...
  CartoonRectangle() = default;
  CartoonRectangle(const CartoonRectangle&) = delete;
  CartoonRectangle(CartoonRectangle&&) = default;
  CartoonRectangle& operator=(const CartoonRectangle&) = delete;
  CartoonRectangle& operator=(CartoonRectangle&&) = default;
  ~CartoonRectangle() override = default;
  ...
  /// The only group is CartoonRectangle
...
template <typename BoundaryConditionsBase>
auto CartoonRectangle::transform_boundary_conditions(
    std::array<
        std::variant<std::unique_ptr<BoundaryConditionsBase>,
                     LowerUpperBoundaryCondition<BoundaryConditionsBase>>,
        2>
...
/// \brief Helper struct for CartoonRectangle options parsing so the internal
...
/// means if we parse a CartoonRectangle as a CartonSphere2DOptionsHelper
/// by having an options parsing specialization (so the input file values are
/// the helper's construtor, not CartoonRectangle's), we can detect the
/// cartoon-type boundary condition for the given system and only then call
/// the CartoonRectangle constructor with the extra information.
struct CartoonRectangleOptionsHelper {
  template <typename Metavariables>
  using options = typename domain::creators::CartoonRectangle::template options<
      Metavariables>;

  template <typename BoundaryConditionsBase>
  using LowerUpperBoundaryCondition =
      domain::creators::CartoonRectangle::LowerUpperBoundaryCondition<
          BoundaryConditionsBase>;

  static constexpr Options::String help = {"CartoonRectangleOptionsHelper"};

  // Default constructor required by Options system
  CartoonRectangleOptionsHelper() = default;

  // Does not take cartoon BC
  CartoonRectangleOptionsHelper(
      std::array<double, 2> lower_bounds, std::array<double, 2> upper_bounds,
      std::array<size_t, 2> initial_refinement_levels,
      std::array<size_t, 2> initial_num_points,
      std::array<CoordinateMaps::Distribution, 2> distributions,
  ...
  CartoonRectangleOptionsHelper(
      ...
      : CartoonRectangleOptionsHelper(
            ...
            domain::creators::CartoonRectangle::transform_boundary_conditions(
                std::move(boundary_conditions)),
            std::move(context)) {}
  // Same members as in CartoonRectangle; public, to be extracted
  std::array<double, 2> lower_bounds_{};
  std::array<double, 2> upper_bounds_{};
  std::array<size_t, 2> initial_refinement_levels_{};
  std::array<size_t, 2> initial_num_points_{};
...
// Options parsing specialization to automate CartoonRectangle's cartoon
// boundary condition
template <>
struct Options::create_from_yaml<domain::creators::CartoonRectangle> {
  template <typename Metavariables>
  static domain::creators::CartoonRectangle create(
      const Options::Option& options) {
    auto helper =
        options.parse_as<domain::creators::detail::CartoonRectangleOptionsHelper,
                         Metavariables>();
    ...
    // Construct CartoonRectangle using the helper's parsed data + cartoon BC
    return domain::creators::CartoonRectangle(
